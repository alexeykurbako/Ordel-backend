...
class ProductService {
    ...
    async updateProduct(product) {
        const preparedProduct = this.mapProductDtoToProduct(product);
        const oldProduct = await this.repository.findById(product.id)

        preparedProduct.image = preparedProduct.image === oldProduct.image ? preparedProduct.image : BASE_BUCKET_URL + await this.uploadImageToS3(preparedProduct.image);
